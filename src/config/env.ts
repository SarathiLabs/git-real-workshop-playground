...
const parsedInterval = Number(import.meta.env.VITE_REFRESH_INTERVAL_MS ?? 0)
...
export const env = {
  ...
  /** 0 (default) means on-demand only. Set a positive ms value to poll. */
  refreshIntervalMs: Number.isFinite(parsedInterval) && parsedInterval > 0 ? parsedInterval : 0,
}
